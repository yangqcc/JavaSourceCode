 ...
 * @since 1.2
 */
public class HashMap<K, V> extends AbstractMap<K, V> implements java.util.Map<K, V>, Cloneable,
    Serializable {
  ...
  /**
   * 默认数组的初始化容量，必须是2的指数 The default initial capacity - MUST be a power of two.
   */
  ...
  /**
   * 容器的数量阈值，找过这个就使用tree，小于该数值就使用 list The bin count threshold for using a tree rather than list for
   * a bin.  Bins are converted to trees when adding an element to a bin with at least this many
   * nodes. The value must be greater than 2 and should be at least 8 to mesh with assumptions in
   *
  ...
  /**
   * table,第一次使用时初始化，必要时扩容，长度通常是2的倍数 The table, initialized on first use, and resized as necessary.
   * When allocated, length is always a power of two. (We also tolerate length zero in some
   * operations to allow bootstrapping mechanics that are currently not needed.)
   */
  ...
  /**
   * 下一次应该扩容的数值，容量*负载因子 The next size value at which to resize (capacity * load factor).
   *
   * @serial
   */
  ...
  /**
   * hash表的负载因子 The load factor for the hash table.
   *
   * @serial
   */
  ...
  /**
   * 指定初始化容量和默认加载因子 Constructs an empty <tt>HashMap</tt> with the specified initial capacity and the
      ...
      for (int j = 0; j < oldCap; ++j) {
        ...
        if ((e = oldTab[j]) != null) {
          ...
          //当前是红黑树
            ...
            do {
              //重置节点通的位置，对于链表来说
              //由于扩容到原来的两倍，所以之前位置的节点可能还是在原来的位置，
              //或者移动到oldCap+j的位置
              next = e.next;
              if ((e.hash & oldCap) == 0) {
                if (loTail == null) {
  ...
  /**
   * Returns a {@link java.util.Set} view of the keys contained in this map. The set is backed by
  ...
  /**
   * Returns a {@link java.util.Set} view of the mappings contained in this map. The set is backed
   * by the map, so changes to the map are reflected in the set, and vice-versa.  If the map is
   * modified while an iteration over the set is in progress (except through the iterator's own
  ...
  static final class TreeNode<K, V> extends LinkedHashMap.Entry<K, V> {
    ...
    /**
     * �������putVal�汾 Tree version of putVal.
     */
     ...
     * Called only from resize; see above discussion about split bits and indices. 只会在扩容时调用该方法
     *
     * @param map the map
     * @param tab the table for recording bin heads 新的table
     * @param index the index of the table being split 节点所在数组位置
     * @param bit the bit of hash to split on   老table容量
     */
