 ...
 * This class obeys the same functional specification as {@link Hashtable}, and includes versions of
 * methods corresponding to each method of {@code Hashtable}. However, even though all operations
 * are thread-safe, retrieval operations do <em>not</em> entail locking, and there is <em>not</em>
 * any support for locking the entire table in a way that prevents all access.  This class is fully
 * interoperable with {@code Hashtable} in programs that rely on its thread safety but not on its
 ...
 * for forEach actions, should ideally be side-effect-free. Bulk operations on {@link Entry} objects
 ...
 * function cannot rely on ordering (more formally, it should be both associative and commutative).
 ...
 * argument. Methods proceed sequentially if the current map size is estimated to be less than the
 * given threshold. Using a value of {@code Long.MAX_VALUE} suppresses all parallelism.  Using a
 * value of {@code 1} results in maximal parallelism by partitioning into enough subtasks to fully
 * utilize the {@link ForkJoinPool#commonPool()} that is used for all parallel computations.
 * Normally, you would initially choose one of these extreme values, and then measure performance of
 * using in-between values that trade off overhead versus throughput.
 *
 * <p>The concurrency properties of bulk operations follow
 * from those of ConcurrentHashMap: Any non-null result returned from {@code get(key)} and related
 * access methods bears a happens-before relation with the associated insertion or update.  The
 * result of any bulk operation reflects the composition of these per-element relations (but is not
 * necessarily atomic with respect to the map as a whole unless it is somehow known to be
 * quiescent).  Conversely, because keys and values in the map are never null, null serves as a
 * reliable atomic indicator of the current lack of any result.  To maintain this property, null
 * serves as an implicit basis for all non-scalar reduction operations. For the double, long, and
 * int versions, the basis should be one that, when combined with any other value, returns that
 * other value (more formally, it should be the identity element for the reduction). Most common
 * reductions have these properties; for example, computing a sum with basis 0 or a minimum with
 * basis MAX_VALUE.
 *
 * <p>Search and transformation functions provided as arguments
 * should similarly return null to indicate the lack of any result (in which case it is not used).
 * In the case of mapped reductions, this also enables transformations to serve as filters,
 * returning null (or, in the case of primitive specializations, the identity basis) if the element
 * should not be combined. You can create compound transformations and filterings by composing them
 * yourself under this "null means there is nothing there now" rule before using them in search or
 * reduce operations.
 *
 * <p>Methods accepting and/or returning Entry arguments maintain
 * key-value associations. They may be useful for example when finding the key for the greatest
 * value. Note that "plain" Entry arguments can be supplied using {@code new
 * AbstractMap.SimpleEntry(k,v)}.
 *
 * <p>Bulk operations may complete abruptly, throwing an
 * exception encountered in the application of a supplied function. Bear in mind when handling such
 * exceptions that other concurrently executing functions could also have thrown exceptions, or
 * would have done so if the first exception had not occurred.
 *
 * <p>Speedups for parallel compared to sequential forms are common
 * but not guaranteed.  Parallel operations involving brief functions on small maps may execute more
 * slowly than sequential forms if the underlying work to parallelize the computation is more
 * expensive than the computation itself.  Similarly, parallelization may not lead to much actual
...
public class ConcurrentHashMap<K, V> extends AbstractMap<K, V>
   ...
   * Accessed directly by iterators. 默认大小为16的数组，用来存储Node节点数据，扩容时大小总是2的幂次方，懒初始化，
   * 在第一次插入时初始化，数量通常是2的倍数
   */
  transient volatile Node<K, V>[] table;

  /**
   * The next table to use; non-null only while resizing. 默认为空，扩容时生成新的数组，其大小为原数组的两倍
   */
   ...
   *
   * table初始化和扩容控制，当为负数时，table表示正在初始化或者扩容，-1表示在初始化， 其他的 -（1+活跃扩容线程），否则，当table为空时，用来表示初始化table的容量，默认
   * 是0。在初始化后，表示下一次将要扩容的table数量
   */
   ...
   *
   * 创建一个新的，空的map，初始化大小为指定数量，而不需要动态的扩容
   *
   ...
   * 如果这个位置为空则直接添加，如果不为空的话，则取出这个节点来 如果取出来的节点的hash值是MOVED(-1)的话，则表示当前正在对这个数组进行扩容，复制到新的数组，则当前线程也去帮助复制
   * 最后一种情况就是，如果这个节点，不为空，也不在扩容，则通过synchronized来加锁，进行添加操作 然后判断当前取出的节点位置存放的是链表还是树
   * 如果是链表的话，则遍历整个链表，直到取出来的节点的key来个要放的key进行比较，如果key相等，并且key的hash值也相等的话，
   * 则说明是同一个key，则覆盖掉value，否则的话则添加到链表的末尾 如果是树的话，则调用putTreeVal方法把这个元素添加到树中去
  ...
  final V putVal(K key, V value, boolean onlyIfAbsent) {
    ...
    for (Node<K, V>[] tab = table; ; ) {
      ...
      if (tab == null || (n = tab.length) == 0) {
        //初始化table
        tab = initTable();
        //当前 table对应位置的节点为空，直接cas新建节点放进去，指定位置
       ...
       */
        ...
        synchronized (f) {
          //再次去除指定位置的元素，和之前的作比较
          if (tabAt(tab, i) == f) {
            //hash值大于0，如果转换为红黑树，那么根节点的hash值为-2
            if (fh >= 0) {
              binCount = 1;
              //遍历整个链表
   ...
   * 一个特殊的Node节点，hash值为-1，其中存储nextTable的引用。只有table
   * 发生扩容的时候，ForwardingNode才会发挥作用，作为一个占位符放在table中表示 当前节点为空或者已经被移动。
   */
  ...
  /**
   * Initializes table, using the size recorded in sizeCtl. 初始化table，使用保存在sizeCtl中的值作为初始化的大小
   */
  private final Node<K, V>[] initTable() {
    Node<K, V>[] tab;
    int sc;
    while ((tab = table) == null || tab.length == 0) {
      //sizeCtl如果小于0，说明正在初始化，那么暂时让出cpu
      if ((sc = sizeCtl) < 0) {
  ...
  /**
   * Helps transfer if a resize is in progress. 如果扩容正在进行，帮助移动
   */
