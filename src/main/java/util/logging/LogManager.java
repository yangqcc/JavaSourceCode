...
public class LogManager {
    ...
    private final static java.util.logging.Level defaultLevel = java.util.logging.Level.INFO;
            ...
            try {
                AccessController.doPrivileged(new PrivilegedAction<Object>() {
                    ...
                    public Object run() {
                        ...
                        final java.util.logging.Logger global = java.util.logging.Logger.global;
    ...
    java.util.logging.Logger demandLogger(String name, String resourceBundleName, Class<?> caller) {
        java.util.logging.Logger result = getLogger(name);
        if (result == null) {
            // only allocate the new logger once
            java.util.logging.Logger newLogger = new java.util.logging.Logger(name, resourceBundleName, caller, this, false);
            do {
    ...
    java.util.logging.Logger demandSystemLogger(String name, String resourceBundleName) {
        // Add a system logger in the system context's namespace
        final java.util.logging.Logger sysLogger = getSystemContext().demandLogger(name, resourceBundleName);
        ...
        if (logger != sysLogger && sysLogger.accessCheckedHandlers().length == 0) {
            ...
            final java.util.logging.Logger l = logger;
            AccessController.doPrivileged(new PrivilegedAction<Void>() {
                @Override
                public Void run() {
                    for (java.util.logging.Handler hdl : l.accessCheckedHandlers()) {
    ...
    class LoggerContext {
        ...
        java.util.logging.Logger demandLogger(String name, String resourceBundleName) {
        ...
        synchronized java.util.logging.Logger findLogger(String name) {
            ...
            java.util.logging.Logger logger = ref.get();
            if (logger == null) {
        ...
        private void ensureAllDefaultLoggers(java.util.logging.Logger logger) {
            if (requiresDefaultLoggers()) {
                ...
                if (!name.isEmpty()) {
                    ensureDefaultLogger(getRootLogger());
                    if (!java.util.logging.Logger.GLOBAL_LOGGER_NAME.equals(name)) {
                        ensureDefaultLogger(getGlobalLogger());
                    }
                }
            }
        ...
        private void ensureDefaultLogger(java.util.logging.Logger logger) {
            ...
            if (!requiresDefaultLoggers() || logger == null
                    || logger != java.util.logging.Logger.global && logger != LogManager.this.rootLogger) {
        ...
        synchronized boolean addLocalLogger(java.util.logging.Logger logger, boolean addDefaultLoggersIfNeeded) {
            ...
            java.util.logging.Level level = owner.getLevelProperty(name + ".level", null);
            ...
            node.loggerRef = ref;
            java.util.logging.Logger parent = null;
        ...
        private void processParentHandlers(final java.util.logging.Logger logger, final String name) {
            final LogManager owner = getOwner();
            AccessController.doPrivileged(new PrivilegedAction<Void>() {
                @Override
                public Void run() {
    ...
    final class SystemLoggerContext extends LoggerContext {
        ...
        @Override
        java.util.logging.Logger demandLogger(String name, String resourceBundleName) {
            java.util.logging.Logger result = findLogger(name);
            if (result == null) {
                // only allocate the new system logger once
                java.util.logging.Logger newLogger = new java.util.logging.Logger(name, resourceBundleName, null, getOwner(), true);
                do {
    ...
    private void loadLoggerHandlers(final java.util.logging.Logger logger, final String name,
            ...
            public Object run() {
                ...
                for (int i = 0; i < names.length; i++) {
                    ...
                    try {
                        ...
                        java.util.logging.Handler hdl = (java.util.logging.Handler) clz.newInstance();
                        // Check if there is a property defining the
                        // this handler's level.
                        String levs = getProperty(word + ".level");
                        if (levs != null) {
                            java.util.logging.Level l = java.util.logging.Level.findLevel(levs);
                            if (l != null) {
                                hdl.setLevel(l);
                            } else {
    ...
    private final ReferenceQueue<java.util.logging.Logger> loggerRefQueue
        = new ReferenceQueue<>();
    ...
    //
    final class LoggerWeakRef extends WeakReference<java.util.logging.Logger> {
        private String                name;       // for namedLoggers cleanup
        private LogNode               node;       // for loggerRef cleanup
        private WeakReference<java.util.logging.Logger> parentRef;  // for kids cleanup
        ...
        void dispose() {
            ...
            if (parentRef != null) {
                ...
                java.util.logging.Logger parent = parentRef.get();
                if (parent != null) {
        ...
        void setParentRef(WeakReference<java.util.logging.Logger> parentRef) {
    ...
    private static void doSetLevel(final java.util.logging.Logger logger, final java.util.logging.Level level) {
        SecurityManager sm = System.getSecurityManager();
        if (sm == null) {
    ...
    private static void doSetParent(final java.util.logging.Logger logger, final java.util.logging.Logger parent) {
        SecurityManager sm = System.getSecurityManager();
        if (sm == null) {
            // There is no security manager, so things are easy.
    ...
    public void reset() throws SecurityException {
        ...
        for (LoggerContext cx : contexts()) {
            ...
            while (enum_.hasMoreElements()) {
                ...
                java.util.logging.Logger logger = cx.findLogger(name);
                if (logger != null) {
                    resetLogger(logger);
                }
            }
    ...
    private void resetLogger(java.util.logging.Logger logger) {
        ...
        java.util.logging.Handler[] targets = logger.getHandlers();
        for (int i = 0; i < targets.length; i++) {
            java.util.logging.Handler h = targets[i];
    ...
    // If the property is not defined or cannot be parsed
    // we return the given default value.
    java.util.logging.Level getLevelProperty(String name, java.util.logging.Level defaultValue) {
        String val = getProperty(name);
        if (val == null) {
            return defaultValue;
        }
        java.util.logging.Level l = java.util.logging.Level.findLevel(val.trim());
    ...
    java.util.logging.Filter getFilterProperty(String name, java.util.logging.Filter defaultValue) {
        String val = getProperty(name);
        try {
            if (val != null) {
    ...
    // property. If the property is not defined or has problems
    // we return the defaultValue.
    java.util.logging.Formatter getFormatterProperty(String name, java.util.logging.Formatter defaultValue) {
        String val = getProperty(name);
        try {
            if (val != null) {
                Class<?> clz = ClassLoader.getSystemClassLoader().loadClass(val);
    ...
    private static class LogNode {
        ...
        void walkAndSetParent(java.util.logging.Logger parent) {
            ...
            while (values.hasNext()) {
                ...
                java.util.logging.Logger logger = (ref == null) ? null : ref.get();
                if (logger == null) {
