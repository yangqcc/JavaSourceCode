...
public class MemoryHandler extends java.util.logging.Handler {
    ...
    private void configure() {
        ...
        pushLevel = manager.getLevelProperty(cname +".push", java.util.logging.Level.SEVERE);
    ...
    public MemoryHandler() {
        ...
        try {
            ...
            target = (java.util.logging.Handler) clz.newInstance();
    ...
    private void init() {
        buffer = new java.util.logging.LogRecord[size];
        start = 0;
        count = 0;
    }
     ...
     */
    public MemoryHandler(Handler target, int size, java.util.logging.Level pushLevel) {
    ...
    public synchronized void push() {
        for (int i = 0; i < count; i++) {
            ...
            java.util.logging.LogRecord record = buffer[ix];
     ...
     *             the caller does not have <tt>LoggingPermission("control")</tt>.
     */
    public synchronized void setPushLevel(java.util.logging.Level newLevel) throws SecurityException {
        if (newLevel == null) {
            throw new NullPointerException();
        }
        checkPermission();
